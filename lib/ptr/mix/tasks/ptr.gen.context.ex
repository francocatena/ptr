defmodule Mix.Tasks.Ptr.Gen.Context do
  ...
  Generates a context with functions around an Ecto schema.
      ...
      mix ptr.gen.context Accounts User users name:string age:integer
  ...
  a `--table` option may be provided. For example:
      ...
      mix ptr.gen.context Accounts User users --table cms_users
  ...
  Read the documentation for `ptr.gen.schema` for more information on
  attributes.
  """
  ...
  def run(args) do
    if Mix.Project.umbrella? do
      Mix.raise "mix ptr.gen.context can only be run inside an application directory"
    end
  ...
  defp inject_schema_access(%Context{file: file} = context, paths, binding) do
    unless Context.pre_existing?(context) do
      Mix.Generator.create_file(file, Mix.Phoenix.eval_from(paths, "priv/templates/ptr.gen.context/context.ex", binding))
    end

    paths
    |> Mix.Phoenix.eval_from("priv/templates/ptr.gen.context/#{schema_access_template(context)}", binding)
  ...
  defp inject_tests(%Context{test_file: test_file} = context, paths, binding) do
    unless Context.pre_existing_tests?(context) do
      Mix.Generator.create_file(test_file, Mix.Phoenix.eval_from(paths, "priv/templates/ptr.gen.context/context_test.exs", binding))
    end

    paths
    |> Mix.Phoenix.eval_from("priv/templates/ptr.gen.context/test_cases.exs", binding)
  ...
  def raise_with_help(msg) do
    ...
    mix ptr.gen.html, ptr.gen.json and ptr.gen.context expect a
    context module name, followed by singular and plural names of
    the generated resource, ending with any number of attributes.
    For example:

        mix ptr.gen.html Accounts User users name:string
        mix ptr.gen.json Accounts User users name:string
        mix ptr.gen.context Accounts User users name:string
